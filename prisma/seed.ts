...
const ACCOUNTS = [
  ...
  },
  {
    ...
    ownerName: "Ifeoma Chukwu",
    bankName: "Moniepoint",
    bankCode: "50515",
  },
  {
    accountNumber: "8888888888",
    ownerName: "James Adeyemi",
    bankName: "Opay",
    bankCode: "999992",
  },
  {
...
async function main() {
  ...
  await prisma.user.upsert({
    where: { email: "admin@snowbank.com" },
    create: {
      name: "SnowBank Admin",
      email: "admin@snowbank.com",
      password: adminPw,
      role: "admin",
  ...
  const demoUser = await prisma.user.upsert({
    where: { email: "demo@snowbank.com" },
    create: {
      name: "Demo User",
      email: "demo@snowbank.com",
      password: demoPw,
      pin: demoPin,
      role: "user",
  ...
  if (!existingSettings) {
    await prisma.appSettings.create({
      data: {
        defaultTransactionLimit: 500000,
  ...
  await prisma.transaction.deleteMany({
    where: { userId: demoUser.id, isGenerated: true },
  });
  ...
  function categoryForType(type: string): string {
    switch (type) {
      ...
      case "deposit":
        return "salary";
      case "transfer":
        return Math.random() > 0.5 ? "transfers" : "shopping";
      default:
        return "others";
    }
  }
  ...
  const txs = Array.from({ length: 50 }, (_, i) => {
    ...
    const direction =
      i < 10 ? "credit" : Math.random() > 0.4 ? "debit" : "credit";
    const type =
      direction === "credit"
        ? Math.random() > 0.5
          ? "deposit"
          : "transfer"
        : ["transfer", "airtime", "bill", "withdrawal"][
  ...
  await prisma.notification.createMany({
    data: [
      {
        userId: demoUser.id,
        title: "Welcome to SnowBank!",
        message: "Your account is ready. Explore our features.",
        type: "system",
      },
      {
        userId: demoUser.id,
        title: "Debit Alert",
        message: "₦15,000 deducted for DSTV subscription",
        type: "transaction",
      },
      {
        userId: demoUser.id,
        title: "Credit Alert",
        message: "₦250,000 salary credit received",
        type: "transaction",
      },
    ],
  });
  ...
  console.log("\n✅ Seeding complete!");
  console.log("Admin: admin@snowbank.com / Admin@123");
  console.log("Demo:  demo@snowbank.com / Demo@123 / PIN: 1234");
}
